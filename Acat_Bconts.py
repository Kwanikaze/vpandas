...
input_dims = {'A': 1,'B': 3} #dicts ordered
cat_vars = ['B']
...
print(sample1_df)
sample1_df_OHE = process.one_hot_encode_columns(sample1_df, cat_vars)
print(sample1_df_OHE)

#  use gpu if available
use_gpu = False
device = torch.device("cuda:0" if use_gpu and torch.cuda.is_available() else "cpu")
VAE_MRF = model.VariationalAutoencoder_MRF(sample1_df_OHE, attributes,input_dims, num_samples,args,cat_vars)
